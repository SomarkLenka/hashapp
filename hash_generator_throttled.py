#!/usr/bin/env python3
"""
Hash generator with BigTable throttling and rate limiting
"""
...
class HashGenerator:
    """Main hash generator with BigTable throttling"""
    ...
    def __init__(self, config: dict):
        ...
        # Rate limiting for BigTable
        self.upload_backoff = 1  # Start with 1 second
        self.max_backoff = 60  # Max 60 seconds
        self.consecutive_failures = 0
        self.last_successful_upload = time.time()
        ...
        # Thread pool - reduced to avoid overwhelming BigTable
    ...
    def bulk_upsert_bigtable_sync(self, batch: List[Tuple[bytes, bytes]]) -> bool:
        """Synchronous BigTable upload with rate limiting"""
        ...
        try:
            ...
            max_batch = 5000  # Only 5k at a time
            batch_to_upload = batch[:max_batch]
            
            rows = []
            for input_hex, hash_bytes in batch_to_upload:
            ...
            # Upload in even smaller chunks
            chunk_size = 1000  # Only 1k mutations at a time
            ...
            self.total_uploaded += len(batch_to_upload)
            
            # Success - reset backoff
            self.consecutive_failures = 0
            self.upload_backoff = 1
            self.last_successful_upload = time.time()
            
            # Log sample hash
            if batch:
                sample = batch[0][1].hex()
                logger.info(f"Sample hash: {sample[:32]}... | Total uploaded: {self.total_uploaded}")
            
            return True
            
        except Exception as e:
            error_msg = str(e)
            if '429' in error_msg or 'throttle' in error_msg.lower():
                # Rate limited - increase backoff
                self.consecutive_failures += 1
                self.upload_backoff = min(self.upload_backoff * 2, self.max_backoff)
                logger.warning(f"Rate limited by BigTable. Backing off for {self.upload_backoff}s")
            else:
    ...
    async def report_stats_to_server(self):
        ...
        try:
            ...
            data = {
                ...
                'upload_backoff': self.upload_backoff
            }
    ...
    async def log_stats(self):
        ...
        if elapsed > 0:
            ...
            logger.info(f"Total Uploaded: {self.total_uploaded:,} | Backoff: {self.upload_backoff}s")
    ...
    async def hash_worker(self, worker_id: int):
        ...
        while True:
            try:
                # Slow down if buffer is too large
                if len(self.batch_buffer) > 1000000:  # 1M max buffer
    ...
    async def periodic_uploader(self):
        """Dedicated uploader task with rate limiting"""
        logger.info("Starting periodic uploader with rate limiting")
        ...
        while True:
            try:
                ...
                time_since_last = time.time() - self.last_successful_upload
                min_interval = max(5, self.upload_backoff)  # At least 5 seconds between uploads
                ...
                should_upload = (
                    len(self.batch_buffer) >= 5000 and  # At least 5k hashes
                ...
                if should_upload:
                    ...
                    # Take small batch to upload
                    batch_size = min(5000, len(self.batch_buffer))
                    batch_to_upload = self.batch_buffer[:batch_size]
                    
                    logger.info(f"Uploading {len(batch_to_upload)} hashes (backoff: {self.upload_backoff}s)...")
                    ...
                    if success:
                        ...
                        logger.warning(f"Upload failed, will retry in {self.upload_backoff}s")
    ...
    async def run(self):
        ...
        logger.info(f"=== STARTING HASH GENERATOR (THROTTLED) ===")
        ...
        logger.info(f"Upload batch size: 5000 (rate limited)")
        logger.info(f"==========================================")
        ...
        # Dedicated uploader with rate limiting
